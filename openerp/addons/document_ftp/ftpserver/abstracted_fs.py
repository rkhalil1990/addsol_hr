...
from openerp.addons.document import document
...
#             pool = pooler.get_pool(node.context.dbname)
            registry = openerp.modules.registry.RegistryManager.get(node.context.dbname)
            object=dir and dir.object or False
            object2=dir and dir.object2 or False
            with registry.cursor() as cr:
                res = registry.get('ir.attachment').search(cr,uid,[('name','like',prefix),('parent_id','=',object and object.type in ('directory','ressource') and object.id or False),('res_id','=',object2 and object2.id or False),('res_model','=',object2 and object2._name or False)])
                if len(res):
                    pre = prefix.split('.')
    ...
    def get_node_cr_uid(self, node):
        ...
        """
        assert node
        db = openerp.modules.registry.RegistryManager.get(node.context.dbname)
